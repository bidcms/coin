...
function _upload($upfile,$uploaddir='',$customfile='',$thumbinfo=array(),$maxsize=1048600)
{
	...
	if($up->checkIsFile() && $up->checkSize() && $up->checkStatus() && $up->checkType())
	{
...
function url($con='index',$act='index',$paramer=array())
{
	...
	else
	{
		...
		{
			$url.=$setting['index_file'];
		}
		elseif($con=='index')
		{
			$url.=$setting['index_file'].'?act='.$act;
		}
		elseif($act=='index')
		{
			$url.=$setting['index_file'].'?con='.$con;
		}
		else
		{
			$url.=$setting['index_file'].'?con='.$con.'&act='.$act;
		}
		if($paramer)
		{
			$p='';
...
function setCode($type)
{
	...
	$code=substr(md5(microtime()),rand(0,10),6);
	$hash=substr(md5(microtime()),rand(0,10),6);
	if($uid>0 && !empty($code))
	{
		mysql_query("delete from bidcms_checkcode where uid=".$uid);
		mysql_query("insert into bidcms_checkcode(type,uid,code,errors,updatetime,hash) values('".$type."',".$uid.",'".$code."',0,".time().",'".$hash."')"); 
		return $hash.$uid;
	}
	return '';
}
//页面转向
function sheader($url,$time=0,$message='',$template='redirect',$admin=false,$target='_self')
{
	if($time>0)
	{
